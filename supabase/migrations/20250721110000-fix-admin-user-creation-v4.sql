-- Migration: Fix Admin User Creation v4 (Resolve Dependencies)
-- This migration fixes the dependency issues and applies all corrections

-- 1. First, drop the trigger that depends on the old function
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;

-- 2. Now we can safely drop the old functions
DROP FUNCTION IF EXISTS link_admin_record_to_user();
DROP FUNCTION IF EXISTS create_admin_user_v2(TEXT, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS get_admin_users_v2();
...
-- Add subscriptions columns if they don't exist
ALTER TABLE subscriptions 
  ADD COLUMN IF NOT EXISTS user_email_registered TEXT,
...
CREATE INDEX IF NOT EXISTS idx_profiles_temp_id ON profiles(temp_id) WHERE temp_id IS NOT NULL;

-- 4. Create the improved admin user creation function
...
-- 5. Create the improved linking function for when user registers
...
-- 7. Create function to get admin users (improved)
...
-- 10. Clean up old data if needed (optional, run with caution)
